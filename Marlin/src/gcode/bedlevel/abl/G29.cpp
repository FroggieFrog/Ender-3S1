...
// G29 Q is also available if debugging
  ...
  if (DISABLED(PROBE_MANUALLY) && seenQ)
    G29_RETURN(false);
#endif
  ...
  if (!g29_in_progress)
  {
    ...
    if (parser.seenval('P'))
      abl.grid_points.x = abl.grid_points.y = parser.value_int();
...
// do_blocking_move_to_z(_MAX(Z_CLEARANCE_BETWEEN_PROBES, Z_CLEARANCE_DEPLOY_PROBE));
  ...
  else
  {
    ...
    if (DEBUGGING(LEVELING))
      DEBUG_ECHOLNPAIR_P(PSTR("Save X"), abl.meshCount.x, SP_Y_STR, abl.meshCount.y, SP_Z_STR, abl.measured_z + abl.Z_offset);
  ...
  while (abl.abl_probe_index < abl.abl_points)
  {
    ...
    if (position_is_reachable(abl.probePos))
      break;
    ++abl.abl_probe_index;
  }
...
#else // !PROBE_MANUALLY
  {
    ...
    for (PR_OUTER_VAR = 0; PR_OUTER_VAR < PR_OUTER_SIZE && !isnan(abl.measured_z); PR_OUTER_VAR++)
    {
      ...
      for (PR_INNER_VAR = inStart; PR_INNER_VAR != inStop; pt_index++, PR_INNER_VAR += inInc)
      {
        ...
        if (abl.verbose_level)
          SERIAL_ECHOLNPAIR("Probing mesh point ", pt_index, "/", abl.abl_points, "."); // pt_index Ŀǰ�ĵ�ƽ�㣬abl.abl_points�ܵĵ�ƽ����
    ...
    {
      if (abl.verbose_level)
        SERIAL_ECHOLNPAIR("Probing point ", i + 1, "/3.");
  ...
  if (!isnan(abl.measured_z))
  {
      ...
      auto print_topo_map = [&](PGM_P const title, const bool get_min)
      {
        ...
        for (int8_t yy = abl.grid_points.y - 1; yy >= 0; yy--)
        {
          LOOP_L_N(xx, abl.grid_points.x)
          {
            ...
            if (get_min)
              NOMORE(min_diff, abl.eqnBVector[ind] - tmp.z);
    ...
    if (!abl.dryrun)
    {
      ...
      if (NEAR(current_position.x, abl.probePos.x - probe.offset_xy.x) && NEAR(current_position.y, abl.probePos.y - probe.offset_xy.y))
      {
